...
#**************************  Completd Home Page  *****************************************************************************
    ...
    if exploration_select=="Geospatial Visualization":
        ...
        country_price = st.multiselect("Select the Country(s)", df["Country"].unique(),default=df["Country"].unique())
        df_country = df[df['Country'].isin(country_price)]
        property_types = st.multiselect("Select the Property Type(s)", df_country["Property_Type"].unique(), default=df_country["Property_Type"].unique())
        df_property_type= df_country[df_country["Property_Type"].isin(property_types)]

        fig_sunburst = px.sunburst(df_property_type, path=["Room_Type", "Bed_type", "Is_Location_Exact"],
        ...
        Neighborhood = st.multiselect("Select the neighborhood(s)", df_country_1["Neighborhood"].unique(), default=df_country_1["Property_Type"].unique())
